...
def main():
    ...
    parser.add_argument("--mode", type=str, default="train",
                       choices=["train", "search", "optuna", "evaluate", "trial"],
                       help="Mode: train, search (random), optuna (Bayesian), evaluate, trial (single config)")
    parser.add_argument("--epochs", type=int, default=MAX_EPOCHS, help="Number of training epochs")
    parser.add_argument("--trials", type=int, default=HYPERPARAM_SEARCH_TRIALS, help="Number of search trials")
    parser.add_argument("--trial-id", type=int, help="Run a specific trial ID (for SLURM job arrays)")
    parser.add_argument("--seed", type=int, default=HYPERPARAM_SEARCH_SEED, help="Random seed")
    ...
    parser.add_argument("--config-file", type=str, help="Path to JSON config list")
    parser.add_argument("--config-index", type=int, help="Index into config file")
    parser.add_argument("--config-json", type=str, help="Inline JSON config")
    parser.add_argument("--dump-configs", type=str, help="Write sampled configs to JSON and exit")
    parser.add_argument("--run-id", type=int, help="Explicit run ID override")
    parser.add_argument("--wandb-group", type=str, help="Wandb group override")
    parser.add_argument("--data-limit", type=int, help="Limit number of games for testing")
    parser.add_argument("--debug", action="store_true", help="Debug mode: 10 games, no shuffle, 50 epochs")
    parser.add_argument("--study-name", type=str, default="hybrid_model_search",
                       help="Optuna study name (for SQLite storage)")
    parser.add_argument("--storage-path", type=str, help="Path to Optuna SQLite database")
